...
public class SwiftFjVibrationPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "fj_vibration", binaryMessenger: registrar.messenger())
        let instance = SwiftFjVibrationPlugin()

        if #available(iOS 13.0, *) {
            SwiftFjVibrationPlugin.createEngine()
        }
    ...
    public static func createEngine() {
        ...
        do {
            SwiftFjVibrationPlugin.engine = try CHHapticEngine()
        } catch {
            print("Engine creation error: \(error)")
            return
        }

        if SwiftFjVibrationPlugin.engine == nil {
            print("Failed to create engine!")
        }

        // The stopped handler alerts you of engine stoppage due to external causes.
        SwiftFjVibrationPlugin.engine?.stoppedHandler = { reason in
            print("The engine stopped for reason: \(reason.rawValue)")
        }

        // The reset handler provides an opportunity for your app to restart the engine in case of failure.
        SwiftFjVibrationPlugin.engine?.resetHandler = {
            // Try restarting the engine.
            print("The engine reset --> Restarting now!")

            do {
                try SwiftFjVibrationPlugin.engine?.start()
            } catch {
    ...
    private func playPattern(myArgs: [String: Any], pattern: [Int]) -> Void {
        ...
        // Try to play engine
        do {
            if let engine = SwiftFjVibrationPlugin.engine {
